                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-body p-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold text-navy">Jenis Kelamin</label>
                                <p class="form-control-static detail-value">{{ formatGender(user.gender) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <form @submit.prevent="submitPasswordChange">
                            ...
                            <div class="d-flex justify-content-end pt-2">
                                <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm w-100-mobile" :disabled="isSubmitting">
                                    <i class="fa fa-save me-1"></i> Simpan Password Baru
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div v-if="user.group == 1 && user.data_katekumen">
                    <div class="card shadow-sm border-0 rounded-4 mb-4 fade-in">
                        <div class="card-body p-4">
                            ...
                            <h6 class="fw-bold text-secondary mb-3 pt-3">Detail Umum Katekumen</h6>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                            ...
                            <div v-if="user.data_riwayat" class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold text-navy">Agama Awal</label>
                            ...
                            <div class="row">
                                ...
                                <div v-if="user.data_menikah?.statusMarried && user.data_menikah.statusMarried !== 'Belum Menikah' && user.data_menikah.statusMarried !== 'Pernah Menikah'" class="col-md-6 mb-3">
                                ...
                                <div v-if="user.data_menikah?.statusMarried && user.data_menikah.statusMarried !== 'Belum Menikah' && user.data_menikah.statusMarried !== 'Pernah Menikah' && user.data_menikah.statusMarried !== 'Menikah Sipil'" class="col-md-6 mb-3">
                                    <label class="form-label fw-bold text-navy">Agama Pasangan</label>
                                    <p class="form-control-static detail-value">{{ user.data_menikah.religionPasangan || '-' }}</p>
                                </div>
                            </div>
                            
                            <div v-if="user.data_menikah?.statusMarried && user.data_menikah.statusMarried !== 'Belum Menikah'" class="p-3 border rounded bg-light-subtle mt-2">
                                ...
                                <div v-if="user.data_menikah.statusMarried === 'Menikah Katolik'">
                                    ...
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Peneguh:</strong> {{ user.data_menikah.namePeneguh1 || '-' }}</p>
                                ...
                                <div v-else-if="user.data_menikah.statusMarried === 'Menikah Kristen'">
                                    ...
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Pendeta:</strong> {{ user.data_menikah.namePeneguh2 || '-' }}</p>
                                ...
                                <div v-else-if="user.data_menikah.statusMarried === 'Menikah Sipil'">
                                    ...
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Tanggal:</strong> {{ formatDate(user.data_menikah.dateMarried3) || '-' }}</p>
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">No. Akta:</strong> {{ user.data_menikah.numberMarried3 || '-' }}</p>
                                </div>
                                <div v-else-if="user.data_menikah.statusMarried === 'Menikah Lain'">
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Tempat/Kota:</strong> {{ user.data_menikah.placeMarried4 || '-' }} / {{ user.data_menikah.cityMarried4 || '-' }}</p>
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Tanggal:</strong> {{ formatDate(user.data_menikah.dateMarried4) || '-' }}</p>
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Peneguh:</strong> {{ user.data_menikah.namePeneguh4 || '-' }}</p>
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">No. Surat:</strong> {{ user.data_menikah.numberMarried4 || '-' }}</p>
                                </div>
                                <div v-else-if="user.data_menikah.statusMarried === 'Pernah Menikah'">
                                    ...
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Kota/Tahun:</strong> {{ user.data_menikah.cityMantan || '-' }} / {{ user.data_menikah.yearMantan || '-' }}</p>
                                    <p class="mb-1 small"><strong class="text-muted me-2 mobile-label-width">Status Akhir:</strong> {{ user.data_menikah.statusMantan || '-' }}</p>
                                </div>
                                <div v-else class="text-muted fst-italic small">Detail pernikahan lanjutan tidak tersedia.</div>
                            </div>
                        ...
                        <div v-if="user.data_keluarga && user.data_keluarga.length > 0" class="row">
                            <div v-for="(family, fIndex) in user.data_keluarga" :key="fIndex" class="col-md-6 mb-3">
                                <div class="card card-body bg-light h-100 p-3 shadow-sm border-0">
                                    <span class="fw-bold text-dark family-name-mobile mb-1 d-block">{{ family.name }}</span>
                                    <small class="text-secondary mb-1 d-block family-detail-mobile">Hubungan: <strong>{{ family.relation || '-' }}</strong> | Agama: {{ family.religion || '-' }}</small>
                                    <small class="text-muted d-block family-detail-mobile">Kontak: {{ family.contact || '-' }}</small>
                                    <small class="text-muted d-block family-detail-mobile">Alamat: {{ family.address || '-' }}</small>
                                </div>
                            </div>
                        </div>
...
const formatGender = (val) => {
    if (val === 1 || val === 'Laki-laki') return 'Laki-laki';
    if (val === 0 || val === 'Perempuan') return 'Perempuan';
    return '-';
};
...
const getGroupName = (groupValue) => {
    if (groupValue === 1 || groupValue === '1') return 'Katekumen';
    if (groupValue === 0 || groupValue === '0') return 'Sakramen Baptis Bayi';
    return 'Tidak Ditentukan';
};

const getStatusBadgeClass = (status) => {
    switch (status) {
        case 'verified': case 'lunas': case 'confirmed': return 'text-success'; 
        case 'confirm': case 'perlu_verifikasi': return 'text-warning';
...
const formatStatus = (status) => {
    switch (status) {
        case 'confirmed': case 'verified': return 'Terverifikasi';
...
const passwordFieldType = reactive({ password: 'password', password_confirmation: 'password' });
...
const submitPasswordChange = () => {
    ...
    if ((form.password || form.password_confirmation) && form.password !== form.password_confirmation) {
        Swal.fire({ title: "Gagal!", text: "Password Baru dan Konfirmasi Password harus sama.", icon: "error", confirmButtonColor: '#003366' });
        return;
    }
    
    if (!form.password && !form.password_confirmation) {
        Swal.fire({ title: "Peringatan", text: "Kolom Password Baru harus diisi jika Anda ingin mengubah password.", icon: "warning", confirmButtonColor: '#003366' });
        return;
    }

    isSubmitting.value = true;

    Inertia.put(`/user/data-profile/${props.user.id}/update-password`, {
        ...
        onSuccess: () => {
            ...
            Swal.fire({ title: "Berhasil!", text: "Password berhasil diperbarui.", icon: "success", showConfirmButton: false, timer: 2000, confirmButtonColor: '#003366' });
        },
        onError: () => {
            isSubmitting.value = false;
            Swal.fire({ title: "Gagal Update", text: "Terdapat kesalahan validasi. Silakan periksa kolom.", icon: "error", confirmButtonColor: '#003366' });
        },
...
/* Read-only styles */
.form-control-static {
    display: block; padding: 0.7rem 1rem; font-size: 0.95rem; line-height: 1.5;
    color: #495057; background-color: #f8f9fa; border: 1px solid #e9ecef; 
    border-radius: 8px; margin-top: 0.2rem; word-wrap: break-word; 
}
.form-label { margin-bottom: 0.1rem; font-size: 0.9rem; }
.detail-value { min-height: calc(1.5em + 1.4rem + 2px); }

/* Buttons */
.btn-primary { background-color: #003366; color: white; border: none; transition: all 0.2s ease; }
.btn-primary:hover:not(:disabled) { background-color: #002244; transform: translateY(-1px); box-shadow: 0 4px 8px rgba(0, 51, 102, 0.3) !important; }

/* Errors */
.error-message { color: #dc3545; background-color: #f8d7da; border: 1px solid #f5c6cb; padding: 0.5rem 1rem; border-radius: 0.5rem; font-size: 0.875rem; }

/* Mobile Responsive */
@media (max-width: 767.98px) {
    .container-fluid.px-4.py-4 { padding: 1rem !important; }
    .header-mobile-stack { flex-direction: column; align-items: flex-start !important; margin-bottom: 1rem !important; }
    .mobile-fs-large { font-size: 1.5rem !important; }
    .mobile-fs-small { font-size: 0.85rem !important; }
    .mobile-status-block { width: 100%; }
    .text-muted.mb-0.small.text-end { text-align: left !important; margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid #eee; }
    
    .card-body.p-4 { padding: 1.25rem !important; }
    .fw-bold.text-navy.mb-4.border-bottom.pb-2 { font-size: 1.1rem; padding-bottom: 0.5rem !important; margin-bottom: 1rem !important; }
    
    .form-label.fw-bold.text-navy { font-size: 0.8rem; }
    .form-control-static.detail-value { padding: 0.5rem 0.7rem; font-size: 0.875rem; min-height: auto; }
    
    .fw-bold.text-secondary.mb-3.pt-3 { font-size: 1rem; margin-top: 1rem !important; }
    .p-3.border.rounded.bg-light-subtle.mt-3 { padding: 1rem !important; }
    .p-3.border.rounded.bg-light-subtle.mt-3 p.small { font-size: 0.85rem; line-height: 1.4; }
    .mobile-label-width { width: auto !important; display: block !important; margin-bottom: 0 !important; }

    .w-100-mobile { width: 100% !important; }
    
    .card.card-body.bg-light.h-100.p-3 { padding: 1rem !important; }
    .family-name-mobile { font-size: 0.9rem; }
    .family-detail-mobile { font-size: 0.75rem; }
}
</style>